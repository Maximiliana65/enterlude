...
- 想要傳送時，只要點擊角落的 🔒 徽章（或按下 `Ctrl+Shift+U`）即可解鎖
  - 解鎖是為了**放行下一次傳送操作**；該操作後會自動重新鎖定
  - 按下 `Esc` 可以取消解鎖、不進行傳送
- 在支援服務中，Enterlude辨識為保護對象的傳送鈕與「重新產生／重試」鈕可避免誤觸。若服務顯示需要額外選擇的選單，明確選取最終項目即可執行
- 在 ChatGPT 中，Enterlude 會尊重 ChatGPT 本身的 Enter 鍵設定。若 ChatGPT 設定為按 Enter 換行，解鎖後按 Enter 換行不會消耗解鎖狀態，也不會顯示趣味留言
- （選用）每次傳送後，可以顯示一句俏皮的小提示 — 預設關閉

若要開啟設定，請點擊瀏覽器工具列上的「擴充功能」圖示（拼圖形狀），再選擇 Enterlude。您可以在設定畫面中開啟或關閉趣味留言，並選擇顯示語言。
