...
| ChatGPT | MAIN worldで止め、サイト本来の`Shift+Enter`処理を発火して改行 | 通過させ、直後に再ロック | MAIN worldで`click`を止める／通す | ProseMirrorへ文字列改行を直接入れると空白扱いになることがある |
...
ClaudeとChatGPTは、解除中のEnterまたは保護対象ボタンを通すと、その操作を1回分としてすぐに再ロックします。Geminiの解除中Enterは、入力欄が空になったこと（送信開始）を`MutationObserver`で確認してから再ロックします。Geminiが送信を受け付けなかった場合は、最大5秒の観察後も解除状態を保ち、Escなどで通常どおり戻せます。

Geminiの送信・再試行ボタンは、解除中にクリックを通した時点で1回分として再ロックします。一方、サービスが表示する追加選択メニューの最終項目はEnterludeの保護対象ではありません。これは、最初の「再試行」を明示的に解除して押した後の追加選択を妨げないためです。
...
- Escで送信せず解除を取り消せること
- 日本語IMEの確定が妨げられないこと
- 送信ボタンと通常の再試行ボタンがロック中に止まり、解除後に1回だけ動くこと
